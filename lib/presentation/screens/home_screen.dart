...
class _HomeScreenState extends State<HomeScreen> {
  ...
  UiOrder _mapServerToUi(sm.Order s) {
    ...
    final priority =
        ...
        ? 'High Priority'
        : 'Normal';
    final address = s.deliveryAddress ?? s.pickupAddress ?? '—';
  ...
  Future<void> _onRefresh() async {
    ...
    // wait for the result (no indefinite waiting)
    await _orderBloc.stream.firstWhere((s) => s is! OrderLoadInProgress);
  }

  @override
  Widget build(BuildContext context) {
    // Listen to AuthBloc so we can react when user logs in/out or changes
    return BlocListener<AuthBloc, AuthState>(
      listener: (context, authState) {
        if (authState is AuthAuthenticated) {
          _orderBloc.add(OrderFetched(id: authState.employee.id));
        } else {
          // Optionally clear list or fetch public/default orders
          // _orderBloc.add(const OrderClear());
        }
      },
